...
pub const DecorationSurface = struct {
    ...
    fn createTitlebarBuffer(self: *Self) !void {
        ...
        self.titlebar_buffer = try buffer.Buffer.init(self.allocator, buffer_size, titlebar_width, titlebar_height, buffer.ShmFormat.argb8888);
    ...
    fn createBorderBuffers(self: *Self) !void {
        ...
        self.border_buffers[0] = try buffer.Buffer.init(self.allocator, top_size, self.width, border_width, buffer.ShmFormat.argb8888);

        // Right border
        const right_size = border_width * self.height * 4;
        self.border_buffers[1] = try buffer.Buffer.init(self.allocator, right_size, border_width, self.height, buffer.ShmFormat.argb8888);

        // Bottom border
        const bottom_size = self.width * border_width * 4;
        self.border_buffers[2] = try buffer.Buffer.init(self.allocator, bottom_size, self.width, border_width, buffer.ShmFormat.argb8888);

        // Left border
        const left_size = border_width * self.height * 4;
        self.border_buffers[3] = try buffer.Buffer.init(self.allocator, left_size, border_width, self.height, buffer.ShmFormat.argb8888);
    ...
    fn renderButtons(self: *Self, pixels: []u32, width: u32, height: u32) !void {
        ...
        try self.renderButton(pixels, width, height, close_x, button_y, ButtonType.close, Color.fromRgb(255, 100, 100));

        // Maximize button (green)
        const maximize_x = close_x - button_size - 5;
        try self.renderButton(pixels, width, height, maximize_x, button_y, ButtonType.maximize, Color.fromRgb(100, 255, 100));

        // Minimize button (yellow)
        const minimize_x = maximize_x - button_size - 5;
        try self.renderButton(pixels, width, height, minimize_x, button_y, ButtonType.minimize, Color.fromRgb(255, 255, 100));
    }

    fn renderButton(self: *Self, pixels: []u32, width: u32, height: u32, x: u32, y: u32, button_type: ButtonType, color: Color) !void {
        const theme = &self.config.theme;
        const button_size = theme.button_size;
    ...
    pub fn getTitlebarGeometry(self: *Self) struct { width: u32, height: u32 } {
        return .{ .width = self.width, .height = self.config.theme.titlebar_height };
    }
