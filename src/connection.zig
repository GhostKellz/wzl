...
pub const Connection = struct {
    ...
    pub fn connectToWaylandSocket(allocator: std.mem.Allocator) !Connection {
    ...
    pub fn receiveMessage(self: *Self) !protocol.Message {
        ...
        while (bytes_read < @sizeOf(protocol.MessageHeader)) {
            const n = try self.socket.read(self.receive_buffer[bytes_read..]);
